...
import './sign.css'
export default function DoorSignup() {
...
const handleMouseDownPassword = () => setShowPassword(!showPassword);
    ...
    const handleSubmit = async () => {
        if (isError == false && verified == true) {
            ...
            if (result.status == 'true') {
                ...
                var result = await postData('door/customerLogin', formData, true)
                Swal.fire({
                    position: 'center',
                    icon: 'success',
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        navigate('/doorlogin',{state:{id:tagId}})
    ...
    const handleopenotpdailog = async () => {
        ...
        const apiUrl = `https://soft7.in/api/send?number=91${phoneNo}&type=text&message=Your Otp For Door Tag By Digital Card Hub - ${otpval}&instance_id=65B92B5C6DD7D&access_token=65b928bbcea41`;
