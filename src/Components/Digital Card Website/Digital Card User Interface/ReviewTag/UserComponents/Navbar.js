...
const Navbar = () => {
    ...
    const tagId = window.localStorage.getItem("tagId")


    const fetchUserDetail = async () => {
        var formData = new FormData()
        formData.append("tagId", tagId)
        var result = await postData('review/getUserDataById', formData, true)
    ...
    const handleNavigate = (value) => {
        setAnchorElUser(null);
        navigate('/reviewchangepassword', { state: { data: userData } })
    };
    const handleLogout = () => {
        setAnchorElUser(null);
        window.localStorage.removeItem('User')
        window.localStorage.removeItem('tagId')
