...
const ReviewChangePassword = () => {
    ...
    const handleChange = async () => {
        ...
        var result = await postData('review/updatePassword', formData, true)
        console.log(result)
        if (result.status) {
            Swal.fire({
                position: 'center',
            ...
            <Grid container spacing={2} sx={{ marginTop: 1, marginBottom: 1,display:"flex",justifyContent:"center" }}>
                ...
                <Grid item xs={12} sx={{ height: '20%', display: 'flex', justifyContent: 'center' }}>
                    <Button sx={{
                        ...
                        textAlign: "center",
                        alignItems: "center",
                    }} variant='contained' onClick={() => navigate('/review/dashboard')}><NavigateBeforeIcon />Back</Button>
                </Grid>
...
export default ReviewChangePassword
