...
export default function DisplayAllDoorTag(props) {
  var theme = useTheme();
  ...
  const handleChange = async(tagId,status) => {
   ...
   const response= await postData('door/updateTagStatus',formData,true)
   ...
   const handleDelete=async(tagId)=>{
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response= await postData('door/deleteReviewTag',formData,true)
  ...
  const fetchAllTags = async () => {
    const result = await getData("door/displayalltags");
    console.log(result);
