...
const columns = [
  ...
  columnHelper.accessor('inviteName', {
    header: 'Invite Name',
    size: 120,
  }),
];
...
const DisplayAllInviteLinks = () => {
  ...
  const matches=useMediaQuery(theme.breakpoints.down(700))
    ...
    const FetchAllCategory = async () => {
      var data = await getData("invitelink/display");
    ...
    const handleDelete=async(row)=>{
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const response= await postData('invitelink/delete',formData,true)
...
export default DisplayAllInviteLinks
